...
def train_preprocess(image, label, use_random_flip=True):
    ...
    if use_random_flip:
        ...
        label = tf.cast(label, tf.float32) / H
        image = concat[:,:,0]
...
def train_preprocess_head_tail(image, label, use_random_flip=True):
    ...
    if use_random_flip:
        ...
        mask = tf.sparse_tensor_to_dense(delta1) + mask  + tf.sparse_tensor_to_dense(delta2)
        ...
        labelh = tf.cast(labelh, tf.float32) / H
        labelt = tf.cast(labelt, tf.float32) / H
...
def build_real_data_v2 (img_h, img_w, batch_size, params):
    ...
    labels_df = pd.read_csv('../datasets/datasets/'+ params.dataset + '/normalized_labels_df.csv')
    labels_df = labels_df.set_index('Unnamed: 0')

    total_no_of_tr_files = len(os.listdir('../datasets/datasets/' + params.dataset + '/train/'))

    if no_of_tr_files:
        tr_folder_names = sorted(os.listdir('../datasets/datasets/' + params.dataset + '/train/'))[0:no_of_tr_files]
        labels_tr = np.array(labels_df.loc[tr_folder_names])[:,label_idx_low:label_idx_hi]

    else:
        tr_folder_names = sorted(os.listdir('../datasets/datasets/' + params.dataset + '/train/'))
        no_of_tr_files = len(tr_folder_names)
        labels_tr = np.array(labels_df.loc[tr_folder_names])[:,label_idx_low:label_idx_hi]

    prev_path = '../datasets/datasets/' + params.dataset + '/train/'
    ...
    tst_folder_names = sorted(os.listdir('../datasets/datasets/' + params.dataset + '/val/'))
    prev_path = '../datasets/datasets/' + params.dataset + '/val/'
